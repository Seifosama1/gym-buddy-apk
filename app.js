...
// ─── Stats page tab switcher (Progress ↔ Goals) ──────────
function switchStatsTab(tab) {
  ...
  if (tab === 'progress') {
    ...
    goalsPane.classList.remove('active');
...
// VAPID public key for this app
const VAPID_PUBLIC_KEY = 'BPD8pKTeXMJXf2KxDZ5z3UQkPv6Mp4BJsPuxNgF7rHon7sfP76p-hEGyDjWrdnYnbrcK3vm-pm_JYmqQ6QlB8hk';
...
async function savePushSubscription(sub) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.warn('[Push] Failed to save subscription:', res.status);
    }
  } catch (e) {
    console.warn('[Push] savePushSubscription error:', e);
  }
}
...
// Call this to send a water push from server side
async function scheduleServerWaterPush(message) {
  const user = getAuthUser();
  const session = getAuthSession();
  ...
  try {
    ...
    console.warn('[Push] scheduleServerWaterPush error:', e);
  }
}
...
function renderAuthState() {
  ...
  if (user) {
    ...
    // If already granted, set up push subscription silently
...
function downloadApkFile() {
  ...
  link.href = 'JimBuddy-NativeNotifications.apk';
  link.download = 'JimBuddy-NativeNotifications.apk';
